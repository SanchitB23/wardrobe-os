...
import {
  ...
  toImportPreviewRows,
  validateUploadedCsvImport,
  validateUploadedJsonImport,
  type ImportLookupCounts,
} from "@/features/inventory/services/import.service";
import {
...
export function InventoryImportView() {
  ...
  async function handleCsvFileSelect(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const validation = await validateUploadedCsvImport(
        content,
        csvLookupsQuery.data,
      );

      if (validation.error) {
        setCsvParseError(validation.error.message);
        return;
      }

      setCsvValidatedRows(validation.data ?? []);
    } catch (error) {
      setCsvParseError(
  ...
  async function handleJsonFileSelect(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const content = await file.text();
      const validation = await validateUploadedJsonImport(content);

      if (validation.error || !validation.data) {
        setJsonParseError(
          validation.error?.message ?? "Failed to validate JSON file.",
        );
        return;
      }

      setJsonLookupCounts(validation.data.lookupCounts);
      setJsonValidatedRows(validation.data.rows);
